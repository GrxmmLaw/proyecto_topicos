...
# ==========================================
# Meta: 2 GB por carpeta
TARGET_SIZE_GB = 2.0  

# Tamaño de cada genoma: 4 Millones de bases (~4 MB)
# Esto es realista para bacterias como E. coli o Salmonella
LONGITUD_BASE = 4000000  

# Contenido de GC (Guanina-Citosina) estándar
GC_CONTENT = 0.50        

# CÁLCULO AUTOMÁTICO DE CANTIDAD DE ARCHIVOS
# 2 GB (en bytes) / 4 MB (en bytes) = 500 archivos aprox.
...
def guardar_fna(carpeta, nombre, cabecera, secuencia):
    """Escribe el archivo FASTA con líneas de 80 caracteres."""
...
def main():
    print(f"=== GENERADOR DE DATASETS SINTÉTICOS (AUTOMÁTICO) ===")
    ...
    dirs = {
        "distintos": "dataset_2GB_distintos",
        "similares": "dataset_2GB_similares_cepas",
        "medios":    "dataset_2GB_medios_especies"
    }
    ...
    # -----------------------------------------------------
    # 2. CATÁLOGO SIMILARES (Cepas - 0.1% diferencia)
    # -----------------------------------------------------
    ...
    # Guardamos la semilla como el archivo 0000
    guardar_fna(dirs["similares"], "genome_strain_0000_SEED", "seed_strain", semilla)

    for i in range(1, N_GENOMAS): # Generamos N-1 variantes
        seq_mut = mutar_secuencia(semilla, 0.001) # 0.1% mutación
        guardar_fna(dirs["similares"], f"genome_strain_{i:04d}", f"synth_strain_{i}", seq_mut)
        imprimir_progreso(i, N_GENOMAS, t_fase)

    # -----------------------------------------------------
    # 3. CATÁLOGO MEDIOS (Especies - 3% diferencia)
    # -----------------------------------------------------
    ...
    for i in range(1, N_GENOMAS):
        seq_mut = mutar_secuencia(semilla2, 0.03) # 3% mutación
    ...
    print(f"Revisa las carpetas: {list(dirs.values())}")
